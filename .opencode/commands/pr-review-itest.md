...
- **警告项（🟡）**：仅存在 🟡 项 → "⚠️ 发现 N 条警告，**优先在当前 PR 中修复**。仅当修复确实不可行时，才延期留档到 deferred-items.json。"
- **全部合规（✅）**："✅ 审查通过，建议合并。"

### 第五步之半：延期待办留档（强制执行）

> ⚠️ **此步骤不可跳过。** 流程：**先判断是否必须延期，能修的立刻修，真有必要延期的才留档。**

1. **判断是否必须延期**：对审查报告和评论中的每条警告，先判断「是否必须延期」：
   - **必须延期**（满足以下任一条件）→ 执行步骤 2 留档：
     - 依赖未到位的功能（如等待其他 Phase 的接口定稿）
     - 需跨 Phase 协调的架构变更
     - 改动过大、风险过高，超出当前 PR 的合理范围
   - **无需延期** → 在当前 PR 中立即修复（commit + push）。修复后无需留档。

2. **留档延期项**：仅对判定为必须延期的条目，**立即**写入 `docs/05-planning/deferred-items.json`（AGENTS.md §8.1.5 R14）。
   - `source` 字段：PR review 来源填 `"CodeRabbit"` 或 reviewer 名；自行发现填 `"Sisyphus (self-review)"`
   - 输出确认信息：
     ```
     ✅ 已将 N 条确认必须延期的条目记录到 deferred-items.json（DEF-XXX ~ DEF-YYY），target_phase=N。
     ✅ M 条警告已判定无需延期，在当前 PR 中修复。
     ```

3. **如果所有问题均无需延期**：跳过留档步骤。
4. ⚠️ **如果在后续步骤中又修复了某条留档项**，将其 `status` 更新为 `resolved`，不要从文件中删除。

### 第六步：处理 PR 中的评论（CodeRabbit / 人工 review）

> PR 中可能有 CodeRabbit 等 AI 审查机器人或人工 reviewer 的评论。需逐条分析并处理。
>
> ⚠️ **核心原则：先判断是否合理，再决定如何修复。** CodeRabbit 的发现不总是准确的——可能是误报、已过时的代码引用、或正则匹配的巧合。每条评论必须经过人工级评估，确认问题确实存在后再动手修复。

**决策流程**（每条评论按此顺序判定）：

```
评论 → 读取相关代码 → 逐条评估是否合理？
  ├─ 不合理（误报/噪声/代码已变更）→ 输出评估结论 → hide
  └─ 合理（确实存在问题）→ 判断是否必须延期？
       ├─ 不必延期 → 立刻修复 → 验证通过 → resolve conversation
       └─ 必须延期（外部依赖、跨 Phase、改动过大）
            ├─ 评论 "Deferred"
            ├─ 写入 deferred-items.json
...
2. **逐条合理性评估（不可跳过，修复前强制执行）**：
   对每条评论，**必须先读取评论指向的实际代码**，再输出评估结论。禁止跳过代码验证直接假定评论合理。
   
   每条的评估依据：
   - **代码验证**：实际读取评论指向的文件和行号，确认问题是否确实存在（代码可能已变更、评论可能引用了旧代码）
   - **AGENTS.md 红线 R1-R14 和质量门禁 G1-G7**
   - **对应的 AC 原文**：评论的建议是否与规格一致？是否会引入规格外的行为？
   - **技术选型文档**：评论建议的方案是否符合既定技术选型？
   
   **评估输出格式**（每条评论必须包含此表，在修复前输出）：
   ```markdown
   ## 📋 CodeRabbit 发现逐条评估
   | # | 评论摘要 | 代码验证 | 合理性 | 依据 | 处理决策 |
   |---|---------|---------|--------|------|---------|
   | 1 | xxx | ✅ 问题存在 / ❌ 误报 | ✅ 合理 / ❌ 不合理 | [引用 AC/红线/实测] | 修复 / 延期(DEF-NNN) / 忽略 |
   ```
   
   - **「代码验证」列**：必须基于实际读取的代码内容判断，不能仅凭评论摘要
   - **「合理性」列**：
     - ✅ 合理：问题确实存在，与 AC/红线/架构一致
     - ❌ 不合理：误报（如正则在测试 fixture 上的巧合匹配）、代码已变更致使评论失效、建议与规格冲突、修改成本远超收益
   - 仅 ✅ 合理的评论进入下一步分流处理；❌ 不合理的直接 hide 或回复说明后 resolve
