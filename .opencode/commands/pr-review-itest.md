...
> ⚠️ **此步骤不可跳过。** 对**延期到后续阶段修复**的问题执行留档；能够立即修复的应在当前 PR 中直接修掉。
> **来源不限**：无论是审查报告 🟡 警告、CodeRabbit/人类 review 评论，还是自行检查中发现的规格偏离或设计权衡 —— **一律写入** `deferred-items.json`。

1. **识别延期项**：从第四步审查报告、第六步评论分析、以及自行发现的问题中筛选，判断哪些可以立即修复、哪些需延期。
   - **可立即修复**：在当前 PR 中修改代码修复 → 第三步或第六步 §3a 流程（commit + push）
   - **需延期修复**：依赖未到位的功能、需跨 Phase 协调、改动过大不适合当前 PR、或有意偏离但需后续对齐 → 留档
2. **留档延期项**：每条需延期的问题**立即**写入 `docs/05-planning/deferred-items.json`（AGENTS.md §8.1.5 R14）。
   - `source` 字段：PR review 来源填 `"CodeRabbit"` 或 reviewer 名；自行发现填 `"Sisyphus (self-review)"`
   - 输出确认信息：
     ```
     ✅ N 条延期项已记录到 deferred-items.json（DEF-XXX ~ DEF-YYY），target_phase=N。
     ```
3. **如果不存在延期项**（全部立即修复或无 🟡 警告）：跳过此步骤。
