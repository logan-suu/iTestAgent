...
> ⚠️ **此步骤不可跳过。** **默认策略：先修复，真修不了再延期。**
> 审查中发现的 🟡 警告/规格偏离/设计权衡，**必须先在当前 PR 中尝试修复**。
> **仅当修复因外部依赖、跨 Phase 协调、改动过大或风险过高确实不可行时，才延期留档。**

1. **先尝试修复**：从第四步审查报告、第六步评论分析、以及自行发现的问题中筛选。
   - **可立即修复**：在当前 PR 中修改代码修复 → 第三步骤或第六步 §3a 流程（commit + push）。**修复后无需留档。**
   - **需延期修复**：仅限以下情况 → 留档
...
2. **留档延期项**：仅对确认无法当前修复的条目，**立即**写入 `docs/05-planning/deferred-items.json`（AGENTS.md §8.1.5 R14）。
   - `source` 字段：PR review 来源填 `"CodeRabbit"` 或 reviewer 名；自行发现填 `"Sisyphus (self-review)"`
   - 输出确认信息：
     ```
     ✅ 已将 N 条确认无法在当前 PR 修复的延期项记录到 deferred-items.json（DEF-XXX ~ DEF-YYY），target_phase=N。
     ✅ M 条警告已在当前 PR 中修复，无需留档。
     ```

3. **如果所有问题均已修复，不存在延期项**：跳过留档步骤。
...
评论 → 有无意义？
  ...
  └─ 有意义 → 先尝试在当前 PR 修复。
       ├─ 修好了 → 验证通过 → resolve conversation
       └─ 确实修不了（外部依赖、跨 Phase、改动过大）→ 延期
            ├─ 评论 "Deferred"
