...
> ⚠️ **此步骤不可跳过。** 仅对**延期到后续阶段修复**的 🟡 警告执行留档；能够立即修复的应在当前 PR 中直接修掉。

1. **识别延期项**：从第四步审查报告中筛选 🟡 警告，判断哪些可以立即修复、哪些需延期。
   - **可立即修复**：在当前 PR 中修改代码修复 → 第六步 §3a 流程（commit + push）
   - **需延期修复**：依赖未到位的功能、需跨 Phase 协调、改动过大不适合当前 PR → 留档
2. **留档延期项**：每条需延期的 🟡 警告**立即**写入 `docs/05-planning/deferred-items.json`，格式见第六步 §3b。
   - 输出确认信息：
     ```
     ✅ N 条延期 🟡 警告已记录到 deferred-items.json（DEF-XXX ~ DEF-YYY），target_phase=N。
     ```
