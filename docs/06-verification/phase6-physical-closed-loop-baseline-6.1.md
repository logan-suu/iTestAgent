...
| US-7.1 | 确认前 metadata-only 解析并锁定 XCUITest 证据候选，确认与权限后走 xcodebuild -> xcresult -> parser -> report；执行后不跨路径 fallback | 部分组件存在，未接生产闭环 | T6.5/T6.8 |
| US-8.1 | 显式选择或 auto 权威确认无 XCUITest 候选时由 DeviceBackend 执行，indeterminate blocked，并禁止从已失败 XCUITest 静默切入 | RED：CLI 固定 launch+screenshot | T6.5/T6.6 |
