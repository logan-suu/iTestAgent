...
```
- 执行前解析可执行 XCUITest 配置（scheme + Test action + test plan? + destination）；hasXCUITest/测试命名不能单独作为调度依据
- 规划期枚举只用 generic platform + `CODE_SIGNING_ALLOWED=NO`，解析 JSON `errors`，不得传具体设备；具体 destination 与 `-allowProvisioningUpdates` 只在计划确认及 `replace_device_app` 权限后使用
