...
- Appium xcodebuild pipeline 在 Xcode 26 + 免费账号下极易失败：
  ...
  **ADR-028 当前约束（2026-09-01 G5）**：Route B/C 均通过同设备主动 readiness/UI/动作/清理验证；Route C 为 production default，Route B 为显式优化路线，禁止静默 fallback。`devicectl info apps` 只能证明 WDA 已安装，不能证明免费 profile 未过期；必须用 route-specific launch + `/status` 或等价 session probe 证明 ready。
```
