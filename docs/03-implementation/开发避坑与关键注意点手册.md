...
- Appium xcodebuild pipeline 在 Xcode 26 + 免费账号下极易失败：
  ...
  xcodebuild CLI 无 Xcode 账号 session、`-allowProvisioningUpdates` 不被 Appium 传递。
   ✅ 解法（ADR-012）：使用 `WdaManager` 自管 WDA 生命周期，通过 `xcodebuild test-without-building -project <path> -scheme <name>`
    绕过 Appium 的 build/install 步骤，Appium 仅用 `usePrebuiltWDA:true + useNewWDA:false` 连接已运行的 WDA。
   G5 真机验证 7/7 PASS 确认此方案可行。
  **Route C（2026-07-25 G5 已验证，推荐）**：使用 `managed-xcodebuild` 模式 + `allowProvisioningDeviceRegistration: true`。
  Appium 管理 xcodebuild 全流程（含 `-allowProvisioningUpdates`），WdaManager 仅负责构建+安装。
  Capabilities: xcodeOrgId + xcodeSigningId(Apple Development) + updatedWDABundleId(基础ID，不带.xctrunner) + allowProvisioningDeviceRegistration(true)
```
