...
```
P1 签名/WDA 首次跑通地狱          最易让首次使用失败、团队 demo 前夜通宵
...
```mermaid
flowchart TD
    P1["P1: 签名/WDA 首次跑通地狱"] --> P3["P3: 无测试+对话类UI"]
    P1 --> D1["🛡️ 强 doctor 引导"]
...
```
- doctor 做成一等公民：诊断 + 逐步引导（签名/DeveloperMode/信任/WDA 首次安装）
- WDA 预构建/预安装，避免每轮现场 build；缓存并校验版本匹配
- Appium + xcuitest-driver + WDA 三者版本锁定，doctor 校验
- 执行前 healthcheck：连接/信任/DeveloperMode/电量/WDA 可用，不满足不进入
- 失败必给“原因+修复步骤”，不静默
- 立项先做“端到端真机 spike”验证签名→装WDA→session→UItree→launch time
```
...
```
- 执行前做测试资产探测；无测试自动转 Appium/WDA Flow
...
- 用真实 .trace 导出 XML 做 fixture，未知 schema 走容错分支不崩溃
```
...
- baseline key 维度：项目+设备型号+iOS+scenario，避免跨设备混用
```
...
```
- 直接 import/fork OpenCode 编排会踩 private 包 + Effect + SQLite 事件溯源 + coding 语义
- 一次性大生成整个循环难以验证
...
```
- 不 import/不 fork，照搬模式自建：AI SDK 多步 tool-calling + MCP tools
- 工具统一 { description, inputSchema, outputSchema, execute } + 输出大小限制
- 先用 mock adapter 跑通循环(idle/续跑)，再接真机
...
- 真机相关必须真机 spike 验证(G5)；纯逻辑用 mock+fixtures
...
启动前先做两个 spike：
```
1. 端到端真机 spike：构建 + 签名 + 装 WDA + Appium 起 session + 抓 UI tree + 截图 + launch time 全链路跑通
...
10 AI 产出走 EPCC-V + G1-G7 门禁，真机必实测
```
...
| v1.1 | 2026-07-13 | 新增 R9-R11、可行性分析 §15、分阶段避坑重点 §14 |
