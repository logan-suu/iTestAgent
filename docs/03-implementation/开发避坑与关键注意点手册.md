...
- result cases、steps.json 与 artifact-index 必须做同 runId 交叉引用校验；case 证据必须关联 relatedCase
...
- 日志/报告入库前脱敏；截图/视频/UI tree 原文仅存当前 run artifacts（T6.8 前可为按 runId 隔离的 staging artifacts）并标注 raw-local-only，目录/文件权限至少为 0700/0600，ArtifactRef 必须指向普通非空文件；提供清理命令；模型、报告嵌入、导出和外传只使用 redacted 派生内容
