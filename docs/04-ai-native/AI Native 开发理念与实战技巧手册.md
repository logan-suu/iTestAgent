...
本手册不是通用 AI 编程科普，而是**结合本项目已有文档（用户故事与验收标准、架构设计、技术选型、数据流全链路）**，把这些设计如何用 AI Native 方式高效、可靠落地讲清楚。
...
理念1 规格即上下文（Spec as Context）
  规格文档就是 AI 的“真理来源”。任何 AI 生成都必须以文档为准，冲突时改文档或纠正 AI，不放任漂移。
...
```
L0 项目宪法      本手册 + AGENTS.md，任何时候优先级最高
...
docs/01-spec/                           规格与需求
docs/02-architecture/                   架构设计、技术选型、数据流
...
Verify   对齐 AC；涉及真机/Simulator 能力走 G5/G5-SIM spike 实测；证据留档
```
...
- 真机相关（签名/backend setup/xctrace/设备）必须 Verify 用真机 spike(G5)；Simulator 相关必须 G5-SIM，禁止"看代码就算过"
```
...
必读上下文：
  ...
  - <相关代码/接口/schema>
硬约束：
  - Local-first、TUI-first、真机与 iOS Simulator 同级支持
...
G5-SIM Simulator 验证  涉及 Simulator 能力必须 CoreSimulator runtime 端到端验证（ADR-011）（不接受“看代码就过”）
...
[ ] 真机/Simulator 能力做 spike 实测(G5 或 G5-SIM)
