...
本手册基于 iOS自动化测试 文件夹下全部文档（项目概述、用户故事与验收标准、架构设计、技术选型、数据流全链路、AI Native 开发手册），系统性汇总 iTestAgent 开发中**最容易踩的坑与关键注意点**，并给出预防和处理办法。
