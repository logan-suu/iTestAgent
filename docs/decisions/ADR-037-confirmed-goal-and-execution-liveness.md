...
### T6.12 性能等待职责澄清（2026-09-08，用户批准）

确认目标及业务wait保留；动作建议另接收确认的memoryObservation与采集启动结果（started/unavailable）。性能观察、操作后settling和采样余量由采集器负责，不能重复编成UI wait；业务wait即使时长相同也保留。采集不可用时仍报告缺失，不能让模型等待或重复工作负载来替代。started只证明启动会话已返回，不保证持续录制或有效采样覆盖。该上下文贯穿生产执行器、TUI/engine默认模型适配器和一次格式纠正；未增加持久化schema或改变权限/动作契约。随后短工作负载G5仅记录一次20秒业务wait，未生成额外70/10秒性能wait；这是单次真实模型行为，不保证后续必定遵守。见性能验证报告§23–24。
