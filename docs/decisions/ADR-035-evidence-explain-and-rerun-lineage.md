...
- PR #78 复审发现，原决策仍把 DeviceBackend 自动探索 case 当成可重放单元，与 US-9.1/ADR-031“只有 confirmed Flow 可复现”冲突；同时 xcresult normalizer 产出的 `Class/Method` 两段 caseId 缺少 xcodebuild 所需的 test target，不能直接作为 `-only-testing` identifier。
...
### 方案 C：两条路径都按逻辑 caseId 再执行（否决）

优点：两条执行路径表面上共享同一筛选模型。

缺点：DeviceBackend 会重新进行模型驱动探索，动作、数据和断言都不等于 parent，无法证明“重跑”；XCUITest 的逻辑 caseId 也不一定含 test target。该方案会把 case 筛选误当成可复现执行。

### 方案 D：XCUITest 使用权威执行标识，DeviceBackend 使用 confirmed Flow（决策）

优点：XCUITest 的 failed-only 与 xcodebuild 原生执行单位一致；DeviceBackend 继续遵守 Flow 才可复现的既有边界；血缘可由 bundle 恢复；flaky 可以按亲子 run 的同一 case 确定性判定。

缺点：T6.9 的 `rerun --failed-only` 不能直接重跑自动探索 case；用户需要先把路径固化为 confirmed Flow，再使用 `itestagent run flow`。未来若要支持 Flow case 级 failed-only，需要先定义 Flow run lineage 与 case-to-flow execution binding，不在本任务内推断。
...
T6.9 只为 `resolvedPath=xcuitest` 创建 child plan。每个 selectedCaseId 必须由 Apple `xcresulttool get test-results tests` 的 `nodeIdentifierURL` 权威恢复 test target、class 和 method，并生成 `Target/Class/Method` 三段标识；`xcresultparser` JUnit 继续提供状态、时长与错误。执行器将三段标识原样作为 `-only-testing` identifier，不能覆盖或冒充 `execution.xcuitest.targets` 的配置 target。只有 JUnit 两段 caseId、legacy caseId、权威节点与 JUnit 无法唯一关联或任何无法证明 target 的标识必须以 typed reason blocked，不能猜测 target 或扩大为全量测试。

`resolvedPath=device_backend` 的自动探索 case 没有 replay identity。`rerun --failed-only` 必须在设备访问和 child run 创建前返回 `rerun_case_not_reproducible`，并给出 `itestagent run flow <flowId>` 指引。重新启动 DeviceBackend 模型探索即使只传同名 caseId，也属于新的探索，不是 parent case 的重跑。

XCUITest child run 复用已确认的 `resolvedPath`，重新执行路径专属 readiness；执行开始后不得跨路径 fallback。命令调用本身表示用户请求执行该已确认计划的 child run，不要求再次确认相同 TestPlan；构建、替换 App、凭证和其他 R7 高风险动作仍必须在本次执行重新经过 PermissionEngine。

### 3. 数据复用遵循安全边界

对 XCUITest child run，“复用原 TestPlan 与数据”表示：

- child plan 复用 parent plan 中已确认并持久化的 XCUITest 配置、非敏感策略和稳定引用，不从执行证据反推测试数据；
- DeviceBackend 的 SecretRef、Keychain 和运行时注入由 confirmed Flow 重放入口按 ADR-033 重新解析，不属于 T6.9 TestPlan child 的数据承诺；
...
- XCUITest failed-only 使用 xcodebuild 可执行的权威 case identity；DeviceBackend 不再把重新探索误报为重跑。
...
- XCUITest 重跑依赖 parser 输出权威 `Target/Class/Method`；历史两段 caseId 必须 blocked，不能推测 target。
- DeviceBackend 自动探索失败不能直接使用 `rerun --failed-only`；可复现路径必须先固化为 confirmed Flow。
...
- XCUITest parser 从权威 target/class/method 生成三段 caseId；两段或歧义标识在设备访问和 child run 创建前被拒绝。
- XCUITest 只把 selectedCaseIds 传给 `-only-testing`，配置 targets 保持不变；G5-SIM 与 G5 分别证明 Simulator 和真机只执行选中 case。
- DeviceBackend 自动探索 parent 返回 `rerun_case_not_reproducible`，不访问设备、不创建 child run；已有 Appium spike 只能证明生产组合与 case 隔离，不能证明可复现重跑。
...
- confirmed Flow 的 SecretRef 仍由 `run flow` 入口重新解析，T6.9 不复制或推断 secret 值。
- runId 在 schema、writer、reader 与 CLI 统一为 1-128 个 `[A-Za-z0-9._-]` 字符。
