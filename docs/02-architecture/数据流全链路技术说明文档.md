...
输出   ProjectAnalysisResult { profile, analysis{analysisTier,enabledCapabilities,limitations} }
...
处理   App 来源优先级：用户指定 > 已有产物 > xcodebuild 构建
      ...
      WDA/Appium 通过 route-specific launch + /status 或等价 session probe 主动验证；失败后确认重签/重装并复验
输出   归一化 App 制品路径 + 安装状态 + WDA readiness（含 route/probe/failure stage）+ 构建日志
...
- WDA 已安装不能产生 ready；launch/tunnel/status/session 任一主动探测失败均 fail-closed，不得静默切换 route
...
- ADR-011 targetKind 引入：test-plan/result/flow 升级到 v2；config 保持兼容
...
S1-S9 数据契约由 B04/B05/B37 的 schema 与 migrations（config-v1/result-v1/artifact-index-v1/test-plan-v2）覆盖，registry shadow-read（B36）保证 compile-time 一致性。
