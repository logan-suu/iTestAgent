...
同一 run 内 `stepId` 与 `sequence` 唯一。参与断言或 case 结果计算的动作必须带 `caseId`，并在该 case 动作完成后立即采集 checkpoint；run 级启动、环境采集与 teardown 可无 `caseId`。Run Steps 始终是实际执行事实，不等同于可重放 Flow（ADR-031）。canonical steps 同时写入 SQLite 查询索引与 run 目录 `steps.json`。
