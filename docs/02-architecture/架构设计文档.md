...
- `AppiumBackend`：**MVP 主 backend，同时支持 physical + simulator（ADR-011）**。T0.2 横评验证真机 session + page source + screenshot 全通过。Physical adapter: devicectl/xcodebuild + Appium/WDA。Simulator adapter: simctl/xcodebuild + Appium/WDA。免费账号可用（workaround 见下）。
