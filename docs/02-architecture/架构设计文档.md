...
flowchart TD
    ...
    Resolve --> Discovery{"executionAssets.status?"}
...
规划期只允许读取 pbxproj、shared scheme/Test action 与 test plan 等元数据，不得执行 Xcode build/test/archive action、scheme pre-action 或项目 Run Script。探测必须输出 `available | none | indeterminate`；只有权威 `none` 才允许 auto 选择 DeviceBackend，`indeterminate` 必须 blocked。确认后执行工程 build/test action先经过 `execute_project_build`，具体 destination 上安装或替换 App/test runner 再经过 `replace_device_app`；真机此时方可传 `-allowProvisioningUpdates`（ADR-030）。
...
`execution.fallback=device_backend` 只适用于 `prefer=auto` 在规划阶段收到权威 `executionAssets.status=none` 的情况。`indeterminate` 必须 blocked。`xcodebuild test` 开始后的构建、测试、超时或解析失败都保留为 XCUITest 路径结果；跨路径重试必须重新确认 TestPlan（ADR-029/030）。
...
`testAssets.hasXCUITest` 是工程图事实，不是生产执行充分条件。当次 `ProjectAnalysisResult.analysis` 必须补充 metadata-only XCUITest 结构候选（scheme、Test action 中的 graph-proven XCUITest targets、可选 test plans、声明的 targetKind 兼容性、evidence、limitations）及 `available | none | indeterminate` 状态。第一版不直接改变持久化 `project-profile.v1`；所选候选写入 TestPlan（ADR-029/030）。
