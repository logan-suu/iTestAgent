...
| `BackendSelector`    | Engine 负责选择和 fallback policy                              | 不直接启动子进程    |
...
不变量：abort 幂等；session 结束后无 pending tool；无 orphan child process；已生成 evidence 仍可索引；ask 可取消且有 timeout；同设备串行、不同设备并行。
