...
| `itestagent-tui`     | 交互式界面（默认 OpenTUI+SolidJS，ADR-008）：对话、计划确认、权限确认、进度、证据浏览；消费生产会话 facade/事件 | 不直接执行 backend 操作或管理底层子进程  |
...
`DeviceDiscoveryProvider.discover(signal?)` 返回 `DeviceDiscoverySnapshot`：`devices + status(ok|partial|failed) + issues[]`。该接口用于目标选择之前，因此不要求预先绑定 UDID，也不经过 BackendSelector；Provider 必须拥有并回收自己启动的短生命周期发现进程。目标确定后，所有设备操作转入 DeviceBackend。
