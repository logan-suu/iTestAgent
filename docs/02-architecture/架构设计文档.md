...
```
依赖 macOS + Xcode 工具链。
依赖签名、Provisioning、Developer Mode、设备信任、WDA 或等价 backend 前置条件。
...
可变的是 backend：mobile-mcp / Appium / iphone-use / XcodeTraceMCP / instrumentsmcp / XcodeProj / XcodeQuery / Drizzle / Kysely 等。
...
Backend 实现层
  mobile-mcp / Appium-WDA / iphone-use / XcodeTraceMCP / instrumentsmcp /
  XcodeProj / XcodeQuery / xcodebuild / xcbeautify / bun:sqlite / Drizzle / Kysely
...
graph TB
    subgraph 交互层["🖥️ 交互层"]
        CLI["itestagent-cli"]
        TUI["itestagent-tui<br/>(OpenTUI/Rezi/Ink)"]
    end
    subgraph 编排层["🧠 编排层"]
...
| `itestagent-tui`     | 交互式界面：对话、计划确认、权限确认、进度、证据浏览         | 不直接调用底层工具  |
...
| `DeviceBackend`      | 真机操作统一接口                                             | 不决定测试目标      |
| `PerformanceBackend` | trace 录制/导出/摘要/baseline 对比                           | 不伪造不可导出指标  |
| `BuildDriver`        | 构建、签名、安装前产物生成、日志格式化                       | 不决定测试策略      |
...
- `MobileMcpBackend`：MVP 第一候选，快速验证完整真机闭环。
- `AppiumBackend`：长期标准 fallback，企业环境更可控。
- `IphoneUseBackend`：WDA tree 差时的视觉/镜像 fallback。
- `CloudDeviceBackend`：Phase 6+ 云真机。
- `MockBackend`：无真机开发和 CI 必备。
...
- `xctrace-analyzer-core`：MVP 第一候选。
- `instrumentsmcp`：录制 + 分析 + report 一体化候选。
- `raw-xcrun-minimal`：fallback。
...
- `raw-xcodebuild`：Apple 官方事实源。
- `xcodequery`：MVP JSON backend 强候选。
- `xcodeproj-helper`：成熟项目图解析。
- `swiftsyntax-helper`：Swift 结构解析。
- `sourcekit/indexstore`：可选语义增强。
...
```
schemaVersion: itestagent.flow.v1
flowId: login-smoke
...
blocked.no_real_device 未发现实体 iPhone
...
| L3 阻断   | Xcode 缺失、Developer Mode 未开、无真机      | 中止 run，输出 doctor 建议           |
...
TUI backend：OpenTUI / Rezi / Ink / headless
...
| 如果                                | 则                                          |
| ----------------------------------- | ------------------------------------------- |
| mobile-mcp 真机稳定且企业环境可用   | MVP 可优先 mobile-mcp backend               |
| mobile-mcp 被 EDR 或 mobilecli 阻断 | AppiumBackend 做主线，mobile-mcp 保留为可选 |
...
| XcodeProj 集成成本高                | XcodeQuery CLI backend 优先                 |
...
MVP 不要一次性实现所有 backend。推荐顺序：

1. MockBackend：保障无真机开发与 CI。
2. mobile-mcp backend：最快验证完整真机产品闭环。
3. AppiumBackend：长期可控 fallback。
...
最终目标：**iTestAgent 是一个 project-aware 的测试语义与证据系统。底层工具可以替换，上层契约必须稳定。**