...
更新时间：2026-09-01（ADR-029 双执行路径解析与语义降级边界同步）
...
stateDiagram-v2
    ...
    awaiting_candidate_confirmation --> awaiting_plan_confirmation: 候选确认
...
flowchart TD
    ...
    Detect --> Resolve["解析可执行 XCUITest 配置<br/>scheme + Test action + test plan? + destination"]
    Resolve --> Preference{"TestPlan preference?"}
    Preference -->|device_backend| PathB["路径 B: DeviceBackend 探索"]
    Preference -->|xcuitest 且唯一可执行| PathA["路径 A: BuildDriver.test"]
    Preference -->|xcuitest 缺失/歧义| Blocked["blocked / 修改并重新确认"]
    Preference -->|auto 且唯一/明确默认| PathA
    Preference -->|auto 且多个歧义| Ask["确认前询问用户"]
    Ask -->|选择 XCUITest 配置| PathA
    Ask -->|选择探索| PathB
    Preference -->|auto 且无可执行配置| PathB
    PathA --> TestReady["scheme/test plan/destination/build/signing readiness"]
    PathB --> DeviceReady["AUT + DeviceBackend healthcheck<br/>Appium 时含 WDA active readiness"]
...
可执行 XCUITest 配置必须证明：scheme 的 Test action 包含至少一个 XCUITest target，可选 test plan 与 scheme 关联，且 destination 与已选 `targetKind` 相容。`ProjectProfile.testAssets.hasXCUITest` 仅表示发现 UI test target，scheme 名称包含 `test` 也不是可执行性证据，二者都不能单独触发生产路径。

规划期证明采用无安装 generic-platform probe：不得传具体设备，必须关闭 code signing，并解析 enumeration JSON 的 `errors`；只有“需要具体设备才能运行”可作为预期 limitation，其余错误全部阻断。具体 destination、签名/provisioning 与 App/test runner 安装属于确认后 readiness/execution，必须经过 `replace_device_app` 权限门禁；真机确认后方可传 `-allowProvisioningUpdates`（ADR-029）。
...
`execution.fallback=device_backend` 只适用于 `prefer=auto` 在规划阶段确认无可执行 XCUITest 配置的情况。`xcodebuild test` 开始后的构建、测试、超时或解析失败都保留为 XCUITest 路径结果；跨路径重试必须重新确认 TestPlan（ADR-029）。
...
`testAssets.hasXCUITest` 是工程图事实，不是生产执行充分条件。当次 `ProjectAnalysisResult.analysis` 必须补充可执行 XCUITest 配置候选（scheme、Test action 中的 XCUITest targets、可选 test plans、targetKind/destination 兼容性、evidence、limitations）。第一版不直接改变持久化 `project-profile.v1`；所选配置写入 TestPlan（ADR-029）。
...
`execution.prefer` 表达用户偏好；`itestagent.test-plan.v3` canonical writer 必须写入 `execution.resolvedPath` 与 `execution.selectionReason`。当解析为 XCUITest 时，`execution.xcuitest.scheme` 必须存在，可选 test plan/过滤条件必须随计划固化。`prefer=auto` 只在确认前解析；确认后不得因运行失败改成另一执行路径。旧 v1/v2 TestPlan 按 ADR-022 迁移；无法无歧义推出路径时 fail-closed（ADR-029）。
