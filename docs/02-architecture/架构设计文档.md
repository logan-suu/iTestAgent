...
同一测试目标的普通多轮消息只补充当前 Planning Cycle，不得隐式重新进入规划起点或使 `confirmed` 状态失效。新测试目标必须由显式用户动作创建新 Planning Cycle，并原子清理旧 Intent、候选与草稿引用。`cancelled` 是当前周期终态，不允许通过候选确认或计划修改原地恢复。
