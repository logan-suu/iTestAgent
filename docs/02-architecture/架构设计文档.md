...
- `AppiumBackend`：**MVP 主 backend，同时支持 physical + simulator（ADR-011）**。T0.2 横评验证真机 session + page source + screenshot 全通过。G5 真机验证 7/7 PASS（Task 3.7, iPhone 14 Plus iOS 18.2.1）。**ADR-012**：MVP 物理设备路径为 managed-xcodebuild + allowProvisioningDeviceRegistration（Route C, G5 已验证）。WdaManager 负责 WDA 构建+安装，Appium 管理 xcodebuild 启动（含 provision 更新）。备选：usePreinstalledWDA（Route A，触控超时）、webDriverAgentUrl（Route B，需 iproxy）。Physical adapter: devicectl/xcodebuild + Appium/WDA + WdaManager。Simulator adapter: simctl/xcodebuild + Appium/WDA。
...
> **Appium/WDA 免费账号 route（T0.2b 验证，Route C G5 确认）**：WDA 默认 bundle ID `com.facebook.WebDriverAgentRunner.xctrunner` 已被 Facebook 注册，免费账号无法使用。**Route C（推荐）**：`managed-xcodebuild` + `allowProvisioningDeviceRegistration: true`，`updatedWDABundleId` 设为 `TEAMID.WebDriverAgentRunner`（不带 `.xctrunner`），Appium 通过此 capability 传递 `-allowProvisioningUpdates` 给 xcodebuild。免费账号 profile 7 天过期需定期重建。
