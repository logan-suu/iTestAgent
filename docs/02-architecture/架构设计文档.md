...
> **Appium/WDA 免费账号 route**：WDA 默认 bundle ID `com.facebook.WebDriverAgentRunner.xctrunner` 已被注册，需使用会话注入的唯一 bundle ID。2026-09-01 T6.4 同设备 G5 已验证 Route B/C 均可用，并选择 Route C 为 production default、Route B 为显式优化路线。WDA Runner 出现在设备清单中不能证明 profile 仍有效，readiness 必须主动探测。
